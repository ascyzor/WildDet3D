...
def get_labelany3d_coco_data_cfg(
    data_root: str = "data/labelany3d_coco",
    data_backend: None | ConfigDict = None,
    with_depth: bool = False,
    shape: tuple[int, int] = (800, 1333),
    cache_as_binary: bool = True,
    test_cached_file_path: str = "data/labelany3d_coco/val.pkl",
) -> ConfigDict:
    """Get the default config for LabelAny3D COCO dataset.

    This dataset contains COCO val2017 images with 3D bounding box annotations
    generated by LabelAny3D. It supports 80 COCO object categories.

    Args:
        data_root: Root directory of the dataset.
        data_backend: Data backend configuration (e.g., HDF5, FileIO).
        with_depth: Whether to load pre-computed depth maps (not available).
        shape: Target image shape (height, width) after preprocessing.
        cache_as_binary: Whether to cache the dataset as binary file.
        test_cached_file_path: Path to the cached file.

    Returns:
        ConfigDict: Dataset configuration for DataPipe.
    """
    test_dataset_cfg = class_config(
        LabelAny3DCOCO,
        data_root=data_root,
        dataset_name="LabelAny3D_COCO_val",
        class_map=labelany3d_coco_class_map,
