...
def get_waymo_dataset_cfg(
    ...
    for dataset in datasets:
        det_map = get_waymo_det_map(
            dataset_name=dataset, data_root=data_root
        )
        class_map = get_waymo_class_map(
            dataset_name=dataset, data_root=data_root
        )
...
def get_waymo_train_cfg(
    data_root: str = "data/waymo",
    train_datasets: Sequence[str] = ("Waymo_train",),
    data_backend: None | ConfigDict = None,
...
def get_waymo_test_cfg(
    data_root: str = "data/waymo",
    test_datasets: Sequence[str] = ("Waymo_val",),
    data_backend: None | ConfigDict = None,
    with_depth: bool = False,
    shape: tuple[int, int] = (800, 1333),
    cache_as_binary: bool = True,
) -> ConfigDict:
    """Get the test config for Waymo.
    ...
    Args:
        ...
        cache_as_binary: Whether to cache as binary.
    """
    test_dataset_cfg = get_waymo_dataset_cfg(
        data_root=data_root,
