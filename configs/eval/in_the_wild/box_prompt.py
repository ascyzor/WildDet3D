"""WildDet3D with LingBot-Depth on InTheWild v3 - Box prompt eval (2026-03-22).

Evaluates WildDet3D + LingBot-Depth (freeze21) on InTheWild v3 benchmark
in box_prompt mode: GT 2D boxes as geometry prompts, monocular depth (no GT depth).
Measures pure 3D regression quality with perfect 2D detection.

Canonical rotation: uses canonical_rotation=True for rotation prediction.
Checkpoint: alldata canonical dense ft 12e 4node.
"""
...
def get_config() -> ExperimentConfig:
    """Returns WildDet3D + LingBot canonical box_prompt eval for InTheWild v3."""
    config = get_default_cfg(
        exp_name="wilddet3d_lingbot_depth_freeze21_in_the_wild_box_prompt_v3"
    )
    ...
    itw_test_data_cfg = class_config(
        ...
        preprocess_fn=get_test_transforms_cfg(
            shape=sam3_image_shape, with_depth=False
        ),
    )
